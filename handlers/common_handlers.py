...
async def cmd_start(message: types.Message, state: FSMContext, user_settings: dict, whisper_model: tuple):
    ...
    text = (
        ...
        "Я твій ШІ-асистент, який вміє:\n"
        "1️⃣ 🗣️ Приймати голосові (.ogg) та аудіо (.mp3, .wav, etc.) повідомлення.\n"
        "2️⃣ ✍️ Транскрибувати їх за допомогою Whisper.\n"
        "3️⃣ 💡 Генерувати коротке резюме тексту за допомогою LLM.\n\n"
        f"🔹 Поточна мова транскрибації: {hbold(SUPPORTED_LANGUAGES.get(current_user_settings['language'], 'Авто'))}\n"
        f"🔹 Поточний стиль резюме: {hbold(current_summary_style_name)}\n"
...
async def cmd_help(message: types.Message, user_settings: dict):
    ...
    text = (
        ...
        "📝 <b>Основні функції:</b>\n"
        "- Транскрибація голосових та аудіо повідомлень.\n"
        "- Сумаризація (короткий виклад) транскрибованого або введеного тексту.\n\n"
        "🎤 <b>Як користуватися:</b>\n"
        "1.  Надішліть голосове повідомлення (запис з Telegram) або аудіофайл.\n"
        "2.  Або просто надішліть текстове повідомлення, яке потрібно сумаризувати.\n"
        "3.  Бот автоматично обробить ваш запит і надішле результат.\n\n"
        "⚙️ <b>Налаштування (/settings):</b>\n"
        f"- <b>Мова транскрибації:</b> Зараз \"{current_lang_name}\". Впливає на точність розпізнавання мови. 'Авто' зазвичай працює добре.\n"
        f"- <b>Стиль резюме:</b> Зараз \"{current_style_name}\". Визначає, наскільки детальним буде короткий виклад.\n\n"
        "❌ <b>Скасування:</b> Команда /cancel дозволяє перервати деякі поточні операції (наприклад, вибір налаштувань)."
    )
    await message.answer(text)
...
async def cmd_settings(message: types.Message, state: FSMContext):
    ...
    await message.answer("⚙️ <b>Налаштування бота</b>\n\nОберіть, що ви хочете налаштувати:",
