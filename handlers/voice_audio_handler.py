...
async def process_audio_message(message: types.Message, bot: Bot, user_settings: dict, whisper_model: tuple):
    ...
    try:
        if message.voice:
            ...
            await status_msg.edit_text("⚠️ Помилка: Не можу обробити цей тип файлу.")
            return
        ...
        if not transcription:
            await status_msg.edit_text("⚠️ Не вдалося розпізнати мову або аудіо порожнє.")
            return

        await status_msg.edit_text("💡 Генерую резюме...")
        summary = await generate_summary(transcription, selected_summary_style)

        transcription_header = f"📜 <b>Транскрибація</b> (Мова: {SUPPORTED_LANGUAGES.get(selected_language, 'Авто')}):"
        summary_header = f"💡 <b>Коротке резюме</b> (Стиль: {SUMMARY_STYLES.get(selected_summary_style, {}).get('name', 'Стандартний')}):"
        ...
        await status_msg.edit_text(f"❌ Сталася серйозна помилка при обробці аудіо: {e}")
    finally:
