...
async def handle_text_input(message: types.Message, user_settings: dict):
    ...
    status_msg = await message.answer("💡 Генерую резюме для вашого тексту...")
    ...
    try:
        ...
        summary_header = f"💡 <b>Коротке резюме</b> (Стиль: {hbold(SUMMARY_STYLES.get(selected_summary_style, {}).get('name', 'Стандартний'))}):"
        ...
        if len(full_response_text) <= MAX_MESSAGE_LENGTH:
            ...
            await status_msg.edit_text("✅ Резюме готове! Надсилаю результат частинами...")
        ...
        await status_msg.edit_text(f"❌ Помилка при генерації резюме: {e}")
